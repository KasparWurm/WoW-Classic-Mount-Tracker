...
function applyFilters() {
    ...
    let filtered = MOUNTS.filter(m =>
        ...
        (!classFilter || (Array.isArray(m.classReq) && m.classReq.includes(classFilter))) &&
        (!profession || (Array.isArray(m.profReq) && m.profReq.includes(profession)))
    );
    ...
    filtered.sort((a, b) => {
        switch (sort) {
            ...
            case "expansion-desc":
                return b.expansion.localeCompare(a.expansion);
            case "owned":
                return (state.owned[b.id] ? 1 : 0) - (state.owned[a.id] ? 1 : 0);
            case "notowned":
                return (state.owned[a.id] ? 1 : 0) - (state.owned[b.id] ? 1 : 0);
            default:
                return 0;
        }
    });
